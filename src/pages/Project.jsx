...
const Project = () => {
    ...
    const [getProject, projectIsLoading] = useApi({
        url: '/profile-monitoring-system/project/' + params.id,
        method: 'GET'
    })
    ...
    const [getServerListByProject, serverListByProjectIsLoading] = useApi({
        url: '/profile-monitoring-system/server/get-all',
        data: {
            count: 10000,
            numberPage: 0,
    ...
    const [deleteProject, deleteProjectIsLoading] = useApi({
        url: "/profile-monitoring-system/project/" + params.id + "/delete"
    })

    const tryDeleteProject = () => {
        deleteProject().then(() => {
            setConfirmationModal(false)
            message.success('Проект ' + project.name + ' удален')
    ...
    useEffect(() => {
        ...
        getServerListByProject().then(resp => {
            setProjectServers(resp.result)
            ...
            {projectIsLoading ?
                ...
                serverError ?
                    ...
                    <>
                        <ProjectHeader
                            project={project}
                            callEditModal={() => setModalFormIsOpen(true)}
                            callDeleteModal={() => setConfirmationModal(true)}
                        />
                        <div className="main__content project" id="project">
                            <p className="project__content-title">Серверы на проекте</p>
                            <div className="project__content">
                                {serversError ?
                                    <ErrorOnPage>{serversError}</ErrorOnPage> :
                                    serverListByProjectIsLoading ?
                                        <PageIsLoading/> :
                                        projectServers.length > 0 ?
                                            <Main listOnly={true} parentData={projectServers}/> :
                                            <EmptyPage/>
                                }
                            </div>
                        </div>
                        ...
                        >
                            <ProjectForm
                                ...
                                editProject={project => setProject(project)}
                                sServers={projectServers}
                            />
                        </Modal>
                        <ConfirmationForm
                            visible={confirmationModal}
                            onOK={tryDeleteProject}
                        ...
                        >
                            ...
                            <p className="modal-confirm__title">Удалить Группу <b>{project.name}</b></p>
                        </ConfirmationForm>
                    </>
            }
        </section>
