...
const Groups = () => {
    ...
    const [deleteGroup, deleteGroupIsLoading] = useApi({
        url: "/profile-monitoring-system/monitoring/deleteGroupById?groupId=" + selectedGroup
    })

    const tryDeleteGroup = () => {
        deleteGroup().then(() => {
            setGroups(groups.filter(el => el.id !== selectedGroup))
            setSelectedGroup(0)
    ...
    return (
        <section className="main">
            <GroupsHeader groupsCount={groups.length} callModal={() => setModalFormIsOpen(true)}/>
            <div id="groups" className="main__content groups">
                {groupsIsLoading ?
                    <PageIsLoading/>
                :
                    groups.length > 0 ?
                        groups.map(item => <GroupsItem group={item} key={item.id} deleteGroup={() => {
            ...
            >
                <GroupForm
                    ...
                    editableGroup={groups.filter(el => el.id === selectedGroup)[0]}
                />
            </Modal>
            <ConfirmationForm
            ...
            >
                ...
                <p className="modal-confirm__title">Удалить Группу <b>{selectedGroup && groups.filter((el) => el.id === selectedGroup)[0].name}</b></p>
            </ConfirmationForm>
        </section>
    );
};
