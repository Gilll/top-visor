...
const DetailsThreadServer = ({thread, alertControl, modalConfirm, item}) => {
    const [changeSentInServer, changeSentIsLoading] = useApi({
        method: 'POST',
        url: `/profile-monitoring-system/settings/${thread.id}/recipient/${item.recipientId}/change`,
        data: {
            folderForRecording: item.folderForRecording,
    ...
    return (
        <li className={"details__thread-item" + (item.isAlive ? ' active' : ' disable')} key={item.id}>
            <div className="details__thread-item-header">
                <div className={"home__status-owner " + item.affiliation}>
                ...
                <p className={"home__status-thread" + (item.maxPower > 100 ? ' green' : ' azure')}>{item.maxPower}</p>
                <p className="details__thread-item-name">{item.name}</p>
                <p className="details__thread-item-ip">{cutIp(item.ip)}</p>
            </div>
            <p className="details__thread-item-path">
                ...
                <span>{item.folderForRecording}</span>
            </p>
            <div className="details__thread-item-footer">
                <p className="details__thread-item-desc">{item.purpose || '-'}</p>
                <button type="button"
                        className={"details__thread-server-btn details__thread-btn--min switch-btn" + (item.isActive ? ' active' : ' disable')}
                        onClick={(e) => {
                            e.stopPropagation();
                            modalConfirm.set({
                                style: item.isActive ? 'red' : 'green',
                                titleText: <>{item.isActive ? 'Выключить' : 'Включить'} отправку на сервер <b>{item.name}?</b></>,
                                btnText: item.isActive ? 'Да, выключить' : 'Да, включить',
                                onConfirm: () => {
                                    changeSentInServer().then((resp) => {
                                        modalConfirm.close();

                                        if (resp.status === 'SUCCESS') {
                                            alertControl.set({text: 'Отправка успешно ' + (item.isActive ? 'выключена' : 'включена'), type: 'success', open: true});
...
export default DetailsThreadServer;